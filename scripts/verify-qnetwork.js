...
const q_network_1 = require("../src/math-framework/neural/q-network");
console.log('🔍 Q-Network Implementation Verification\n');
try {
    // Test 1: Matrix Operations
    console.log('Test 1: Matrix Operations');
    const A = q_network_1.Matrix.from2D([[1, 2], [3, 4]]);
    const B = q_network_1.Matrix.from2D([[5, 6], [7, 8]]);
    ...
    const network = new q_network_1.QNetwork({
        layers: [2, 4, 1],
        learningRate: 0.1,
        lambda: 0.1,
        maxIterations: 100,
    ...
    const input = q_network_1.Matrix.from2D([[0.5], [0.5]]);
    ...
    const X = [
        q_network_1.Matrix.from2D([[0], [0]]),
        q_network_1.Matrix.from2D([[1], [1]]),
    ];
    const Y = [
        q_network_1.Matrix.from2D([[0]]),
        q_network_1.Matrix.from2D([[1]]),
    ];
    ...
    for (const traj of result.trajectories) {
        ...
        console.assert(Math.abs(expectedV - actualV) < 1e-10, `V(n) = S(n)² property violated at iteration ${traj.iteration}`);
    }
    ...
    const network2 = new q_network_1.QNetwork({
        layers: [2, 4, 1],
    });
    network2.importWeights(weights);
    const testInput = q_network_1.Matrix.from2D([[0.3], [0.7]]);
...
//# sourceMappingURL=verify-qnetwork.js.map